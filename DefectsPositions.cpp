...
namespace unfolding {
	...
	void DefectsPositions::findConnectivity() {
		...
		for (int i = 0; i < n_def; i++) {
			...
			while (isstop != 2) {
				...
				for (int j = 0; j < n_def; j++) {
					...
					//if(def_i==0)cout << r << " " << def_j << " " << init_r << " " << _delr << endl;
					if (r <= init_r + _delr / 2 && r >= init_r - _delr / 2 && i != j) {
						count_n++;
					}
				}
				//if (def_i == 0)cout << count_n << endl;
		...
		//for(int i=0;i<defect_neighbor.size();i++)print_vector(defect_neighbor[i]);
	}
